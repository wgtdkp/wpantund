...
unpack_bbr_dataset(const uint8_t *data_in, spinel_size_t data_len, boost::any &value, bool as_val_map)
{
	...
	syslog(LOG_INFO, "in unpack_bbr_dataset");
	...
	if (as_val_map) {
		...
		{
			Data data(0);
			data.push_back(dataset.mSequenceNumber.get());
			// pack the reregistration delay in big endian.
			data.push_back(dataset.mReregistrationDelay.get() >> 8);
			data.push_back(dataset.mReregistrationDelay.get() & 0xff);

			// pack the mlr timeout in big endian.
			data.push_back(dataset.mMlrTimeout.get() >> 24);
			data.push_back(dataset.mMlrTimeout.get() >> 16);
			data.push_back(dataset.mMlrTimeout.get() >> 8);
			data.push_back(dataset.mMlrTimeout.get() & 0xff);
			value = data;
		}
	}
...
SpinelNCPInstance::regsiter_all_get_handlers(void)
{
	...
	register_get_handler_spinel_simple(
		...
		SPINEL_PROP_THREAD_PRIMARY_STATE, SPINEL_DATATYPE_UINT8_S);
	register_get_handler_spinel_simple(
		kWPANTUNDProperty_ThreadBbrGroupTable,
		SPINEL_PROP_THREAD_BBR_GROUP_TABLE, SPINEL_DATATYPE_DATA_S);
	register_get_handler_spinel_simple(
		kWPANTUNDProperty_ThreadBbrDuaTable,
		SPINEL_PROP_THREAD_BBR_DUA_TABLE, SPINEL_DATATYPE_DATA_S);
	...
	register_get_handler_spinel_unpacker(
		...
		SPINEL_PROP_THREAD_PRIMARY_BBR_DATASET, boost::bind(unpack_bbr_dataset, _1, _2, _3, /* as_val_map */ true));
	register_get_handler_spinel_unpacker(
...
SpinelNCPInstance::handle_ncp_spinel_value_is(spinel_prop_key_t key, const uint8_t* value_data_ptr, spinel_size_t value_data_len)
{
	...
	if (key == SPINEL_PROP_LAST_STATUS) {
	...
	} else if (key == SPINEL_PROP_THREAD_BBR_GROUP_TABLE) {
		nl::Data data;
		nl::Data cache;
		...
		syslog(LOG_INFO, "[-NCP-] Group: Total %d Multicast Group%s", num_entries, (num_entries > 1) ? "s" : "");
	...
	} else if (key == SPINEL_PROP_THREAD_BBR_DUA_TABLE) {
		nl::Data data;
		nl::Data cache;
		data.push_back(2);
		uint16_t num_entries = 0;

		{
			...
			while (value_data_len > 0)
			{
				...
				syslog(LOG_INFO, "[-NCP-]: DuaTable entry %d", num_entries);
		...
		syslog(LOG_INFO, "[-NCP-] Group: Total %d Dua Entr%s", num_entries, (num_entries > 1) ? "ies" : "y");

		data.push_back(num_entries >> 8);
		data.push_back(num_entries & 0xff);
		data.append(cache);

		signal_property_changed(kWPANTUNDProperty_ThreadBbrDuaTable, data);
		}
	...
	} else if (key == SPINEL_PROP_THREAD_PRIMARY_STATE) {
		uint8_t state;
...
SpinelNCPInstance::handle_ncp_spinel_value_inserted(spinel_prop_key_t key, const uint8_t* value_data_ptr, spinel_size_t value_data_len)
{
	if (key == SPINEL_PROP_IPV6_ADDRESS_TABLE) {
...

		{
			...
			syslog(LOG_INFO, "[-NCP-]: DuaTable 1 entry added");
...
SpinelNCPInstance::handle_ncp_spinel_value_removed(spinel_prop_key_t key, const uint8_t* value_data_ptr, spinel_size_t value_data_len)
{
	if (key == SPINEL_PROP_THREAD_CHILD_TABLE) {
...

		{
			...
			syslog(LOG_INFO, "[-NCP-]: DuaTable 1 entry removed");
